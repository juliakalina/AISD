...
int KS1SearchEmpty(KeySpace1* ks1)
{
...
{
	int release = 0;
	for (int i = 0; i < SPACE_SIZE && ks1->data[i].key; ++i)
		if (!strcmp(ks1->data[i].key, key))
...
{
	for (int i = 0; i < SPACE_SIZE; ++i)
		if (ks1->data[i].key && !strcmp(ks1->data[i].key, key) && ks1->data[i].release == release)
			return i;
...
void KS1Sort(KeySpace1* ks1)
{
	for (int i = 0; i < SPACE_SIZE; ++i)
			for (int j = i + 1; j < SPACE_SIZE; ++j) 
			{
				...
				int cmp;
				if (ks1->data[i].key)
					cmp = strcmp(ks1->data[i].key, ks1->data[j].key);
				else
					cmp = 1;
				...
				if (cmp == 0 && ks1->data[i].release > ks1->data[j].release)
					swap(ks1, i, j);
			}
}

void KS1Print(FILE* file, KeySpace1* ks1)
{
	printf("KeySpace1\n");
	...
	for (int i = 0; i < SPACE_SIZE; ++i)
		if (ks1->data[i].key)
		{
			...
			PrintItem(file, ks1->data[i].item);
